...
class TemplateFileController extends GetxController {
  ...
  RxList<TemplateFile> collection = RxList<TemplateFile>();
  ...
  Future getData({bool isRefresh = false, bool isClearData = true}) async {
    ...
    try {
      var response = await APICaller.getInstance().get(
          'v1/template-file?page=$page&limit=$limit&keyword=${searchController.value.text.trim()}');
