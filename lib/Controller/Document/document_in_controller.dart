...
class DocumentInController extends GetxController {
  ...
  Future getData({bool isRefresh = false, bool isClearData = true}) async {
    ...
    try {
      var response = await APICaller.getInstance().get(
          'v1/document/in?page=$page&limit=$limit&keyword=${searchController.value.text.trim()}');
  ...
  deleteItem(int index) {
    APICaller.getInstance()
        .delete('v1/document/cancel/${collection[index].uuid}')
        .then((response) {
      if (response['code'] == 200) {
        Utils.showSnackBar(title: 'Thông báo', message: response['message']);
        collection[index].status = 0;
        collection.refresh();
      }
    });
  }
