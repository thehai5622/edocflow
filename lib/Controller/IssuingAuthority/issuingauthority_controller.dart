...
class IssuingAuthorityController extends GetxController {
  ...
  RxList<IssuingAuthority> collection = RxList<IssuingAuthority>();
  ...
  Future getData({bool isRefresh = false, bool isClearData = true}) async {
    ...
    try {
      var response = await APICaller.getInstance().get(
          'v1/issuingauthority?page=$page&limit=$limit&keyword=${searchController.value.text.trim()}');
  ...
  deleteItem(int index) {
    APICaller.getInstance().delete('v1/issuingauthority/${collection[index].uuid}').then((response) {
      Utils.showSnackBar(title: 'Thông báo', message: response['message']);
      totalCount.value--;
